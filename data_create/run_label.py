...
LABEL_SAVE_DIR = "run_posture_label"  
SAVE_DIR = "run_posture_dataset" 
SAMPLE_FPS = 5  
CONF_THRESHOLD = 0.5  
NORMALIZE = True                      
TEST_SIZE = 0.2                       
...
def manual_label_frames(video_path,norm_pose_seq,original_pose_seq, frame_list):
    ...
    print("   按 1 键 → 标注为摆动")
    print("   按 2 键 → 标注为落地")
    print("   按 I 键 → 上一帧标注为起跑")
    print("   按 O 键 → 上一帧标注为摆动")
    print("   按 P 键 → 上一帧标注为落地")
    ...
    for i in range(total_frames):
        ...
        while True:
            ...
            if key == 48:  # 数字键0
                labels.append(0)
                print(f"帧{i+1}/{total_frames} → 标注为起跑(0)")
                break
            elif key == 49:  # 数字键1
                labels.append(1)
                print(f"帧{i+1}/{total_frames} → 标注为摆动(1)")
                break
            elif key == 50:  # 数字键2
                labels.append(2)
                print(f"帧{i+1}/{total_frames} → 标注为落地(2)")
                break
            elif key == 73:
                labels[-1] == 0
                print(f"帧{i}/{total_frames} → 标注为起跑(0)")
                continue
            elif key == 79:
                labels[-1] == 1
                print(f"帧{i}/{total_frames} → 标注为摆动(1)")
                continue
            elif key == 81:
                labels[-1] == 1
                print(f"帧{i}/{total_frames} → 标注为落地(2)")
                continue
            elif key == 27:  # ESC键
                print(" ESC跳过，剩余帧标为0")
                labels.extend([0] * (total_frames - len(labels)))
                ...
                print(f"标注完成：{label_save_path} | 起跑={np.sum(labels==0)}, 摆动={np.sum(labels==1)},落地={np.sum(labels==2)}")
                return labels
            elif key == 32:  # 空格键
                pause = True
                ...
                print(f" 无效按键！请按 0/1/2/ESC/空格，当前按键：{key}")
                continue
...
def process_single_video(video_path):
    ...
    for i in range(0,len(norm_pose_seq)):
        x = norm_pose_seq[i]
        X.append(x)
        y.append(labels[i])
...
def print_dataset_info(X_train, y_train, X_test, y_test):
    ...
    print(f"采样频率：{SAMPLE_FPS}帧")
    print(f"\n训练集：总数={len(X_train)} | 起跑={np.sum(y_train==0)} | 摆动={np.sum(y_train==1)} | 落地={np.sum(y_train==2)}")
    print(f"测试集：总数={len(X_test)} | 起跑={np.sum(y_test==0)} | 摆动={np.sum(y_test==1)} | 落地={np.sum(y_test==2)}")
    print("====================================================")
