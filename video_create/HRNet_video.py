...
def generate_video(input_path):
    ...
    vector_array = vector_array.reshape(frame_count, 17, 3)
    # 提取 dx, dy, speed
    dxdy = vector_array[:, :, :2]  # (帧数, 17, 2)
    speed = vector_array[:, :, 2:]  # (帧数, 17, 1)
    # 计算全局最大最小值
    ...
    print(dxdy_normalized)
    speed_normalized = (speed - speed_min) / (speed_max - speed_min + 1e-8)
    # 重新组合
    vector_normalized = np.concatenate([dxdy_normalized, speed_normalized], axis=-1)
