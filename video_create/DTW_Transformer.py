# DTW_test_Transformer.py（适用 Skeleton-Transformer 编码器的评分版本）
# 与 DTW_test_MLP.py 框架完全一致，仅将 MLPEncoder 替换为 SkeletonTransformerEncoder
import cv2
import numpy as np
import matplotlib
...
WINDOWSIZE = 7 #窗口大小
# 改动：指向 Transformer 训练脚本保存的模型
MODEL = 'D:/Dataset/sprint/result/model/Transformer/best_7_1_tran.pth'

# ============ 以下是与 SkeletonTransformer_filter.py 完全一致的模型定义 ============
class COCOGraph:
    """17 关节 COCO 骨架图，与 ST-GCN/Transformer 训练脚本一致"""
...
class SkeletonTransformerEncoder(nn.Module):
    """带骨骼结构先验的 Transformer 编码器（与训练脚本结构完全一致）"""
    ...
    def forward(self, x):
        ...
        x = x + self.temporal_pos_embed      # 时间位置编码
        x = x.reshape(N, V, T * C)           # (N, V, 14)
        x = self.embed(x)                    # (N, V, d_model)
        x = x + self.joint_pos_embed         # 关节位置编码
        for block in self.blocks:
            x = block(x, self.hop_idx)
        x = self.norm(x)
        x = x.mean(dim=1)                    # 关节 token 平均池化
        x = self.projection(x)
...
class VideoScoreEvaluator:
    def __init__(self, 
        ...
        self.kps1 = []
        self.kps2 = []
        self.window_sim_scores = None  # 保存每个窗口的Transformer相似度得分
    ...
    def compute_pairwise_scores(self, path: np.ndarray, use_window=True):
        ...
        if use_window and self.window > 0:
            ...
            # 改动：SkeletonTransformerEncoder，参数必须与训练脚本一致
            model = SkeletonTransformerEncoder(
                in_channels=2, window_size=WINDOWSIZE, num_joints=17, hop_size=2,
                d_model=128, nhead=4, num_layers=2, dim_feedforward=256,
                output_dim=64, dropout=0.2
            ).to(device)
...
def visualize_window(evaluator, window_idx):
    ...
    fig.suptitle(f'Window {window_idx} (Transformer Similarity: {evaluator.window_sim_scores[window_idx]:.2f})',  # 改动：MLP -> Transformer
                    fontsize=14)
