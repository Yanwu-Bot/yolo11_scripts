# SkeletonTransformer_filter.py
# 与 ST-GCN_filter.py 框架完全一致，仅将编码器替换为带骨骼结构先验的 Transformer
import time
import os
import math
import random
...
MODEL_SAVE_N = 'best_7_1_tran.pth'   # 模型保存名，区分于 ST-GCN/MLP 版本
...
class COCOGraph:
    """17 关节 COCO 骨架图，与 ST-GCN 版本完全一致"""
...
def train_contrastive(dataset, epochs=100, batch_size=32, lr=1e-3, temperature=0.5,
                    diversity_threshold=0.0, d_model=128, nhead=4, num_layers=2,
    ...
    model = SkeletonTransformerEncoder(
        ...
        dim_feedforward=dim_feedforward, output_dim=64, dropout=dropout
    ).to(device)
...
if __name__ == '__main__':
    ...
    train_contrastive(
        dataset, epochs=100, batch_size=64, lr=0.001, temperature=0.1,
