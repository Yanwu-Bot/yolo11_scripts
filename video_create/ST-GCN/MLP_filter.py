# MLP.py（简单基线版：窗口展平 -> MLP -> 嵌入）
# 与 ST-GCN_filter.py 框架完全一致，仅将编码器从 ST-GCN 替换为 MLP
import time
import os
import math
import random
...
MODEL_SAVE_N = 'best_7_1_mlp.pth'   # 模型保存名，区分于 ST-GCN 版本
...
def train_contrastive(dataset, epochs=100, batch_size=32, lr=1e-3, temperature=0.5, diversity_threshold=0.0):
    ...
    model = MLPEncoder(output_dim=64).to(device)      # 替换模型
    optimizer = optim.Adam(model.parameters(), lr=lr)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True)
    best_loss = float('inf')
    save_dir = 'D:/Dataset/sprint/result/model/MLP'                     # 分开保存，不覆盖 ST-GCN 的模型
