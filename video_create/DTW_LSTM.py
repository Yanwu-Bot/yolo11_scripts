# DTW_test_LSTM.py（适用LSTM编码器的评分版本）
# 与 DTW_test.py 框架完全一致，仅将 ST-GCN 编码器替换为 LSTMEncoder
import cv2
import numpy as np
import matplotlib
...
WINDOWSIZE = 7 #窗口大小
MODEL = 'D:/Dataset/sprint/result/model/LSTM/best_7_1_lstm.pth'   # 改动：指向 LSTM 训练脚本保存的模型

class LSTMEncoder(nn.Module):
    """
    将 (N, C, T, V) 关节序列编码为 output_dim 维特征并 L2 归一化。
    结构与 LSTM 训练脚本中的 ContrastiveEncoder 保持一致，
    保证能正确加载 best_7_1_lstm.pth 的 state_dict。
    """
    def __init__(self, in_channels=2, hidden_size=128, num_layers=2,output_dim=64, num_joints=17):
        super().__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        # 每个时间步输入 = 所有关节点坐标展平 (V * C)
        self.input_size = num_joints * in_channels   # 17*2 = 34
...
class VideoScoreEvaluator:
    def __init__(self, 
        ...
        self.kps1 = []
        self.kps2 = []
        self.window_sim_scores = None  # 保存每个窗口的LSTM相似度得分
    ...
    def compute_pairwise_scores(self, path: np.ndarray, use_window=True):
        ...
        if use_window and self.window > 0:
            ...
            model = LSTMEncoder(output_dim=64).to(device)   # 改动：LSTMEncoder
...
def visualize_window(evaluator, window_idx):
    ...
    fig.suptitle(f'Window {window_idx} (LSTM Similarity: {evaluator.window_sim_scores[window_idx]:.2f})',  # 改动：ST-GCN/MLP -> LSTM
                    fontsize=14)
