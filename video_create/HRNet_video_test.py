...
All_feature = []                         #用于存放特征
All_point = []                           #用于存放关键点
All_normalized_points = []               #用于存放归一化关键点（新增，替代原来的 Normal_keypoints 和 Vector_list）
...
def compute_displacement_features(normalized_points_array, window_size=WINDOW_SIZE):
    ...
    for t in range(n_frames):
        ...
        displacement_features.append(stacked)
    ...
    if max_val - min_val > 1e-8:
        displacement_features = (displacement_features - min_val) / (max_val - min_val)
    else:
        displacement_features = np.zeros_like(displacement_features)
...
def generate_video(input_path):
    ...
    if All_normalized_points:
        normalized_array = np.stack(All_normalized_points, axis=0)  # (帧数, 17, 2)
        print(f"归一化关键点数组形状: {normalized_array.shape}")
        
        # 计算位移特征（支持窗口堆叠）
        displacement_features = compute_displacement_features(normalized_array, window_size=WINDOW_SIZE)
        print(f"位移特征形状: {displacement_features.shape}")  # (帧数, window_size, 17, 2)
        ...
        # 保存位移特征
        np.save(f'result/features/{video_name}_vector.npy', displacement_features)
        print(f"位移特征已保存: result/features/{video_name}_vector.npy")
    else:
        print("警告: 没有归一化关键点数据")
    # =========================================
